/*
 * ============================================================
 * AIRCRAFT PROTOTIPE — AIS + WiFi Monitor + MicroSD Logger
 ...
 * ============================================================
 *
 * AIS MESSAGE 24 (STATIC DATA) OTOMATIS:
 *   Mengirim MSG 24 Part A dgn nama "AIRCRAFT PROTOTIPE"
 *   setiap 6 menit (standar AIS) agar nama muncul di peta.
 *
...
#define AIS_DAC         366
#define AIS_FI          56
#define SHIP_NAME       "AIRCRAFT PROTOTIPE"
#define INTERVAL_MSG9   10000UL
#define INTERVAL_MSG8   1000UL
#define INTERVAL_MSG24  360000UL
...
unsigned long tMsg9=0, tMsg8=0, tMsg24=0, tWS=0, tSDLog=0;

uint8_t seqMsg9=0, seqMsg8=0, seqMsg24=0;
...
char lastMsg24Payload[220]    = "";
char lastMsg24RawPayload[180] = "";

char lastMsg9FullNMEA[256] = "";
char lastMsg8FullNMEA[256] = "";
char lastMsg24FullNMEA[256] = "";
...
void sdInit() {
  ...
  if (f) {
    f.println(
      ...
      "drone_state,"
      "ais_msg9_full_nmea,ais_msg8_full_nmea,ais_msg24_full_nmea"
    );
    f.close();
...
void sdWriteLog(const char* aisMsg9Full, const char* aisMsg8Full, const char* aisMsg24Full) {
  if (!sdReady) return;
...
// ------------------------------------------------------------
// Tambah teks → 6-bit AIS ASCII per karakter
// ------------------------------------------------------------
void addTextBits(const char *text, uint8_t maxChars) {
  for (uint8_t i = 0; i < maxChars; i++) {
    uint8_t val = 0;
    if (text[i] != '\0') {
      char c = text[i];
      if (c >= 'a' && c <= 'z') val = c - 'a' + 1;
      else if (c >= 'A' && c <= 'Z') val = c - 'A' + 1;
      else if (c == ' ') val = 32;
    }
    addBits(val, 6);
  }
...
// ============================================================
// BUILD AIS MESSAGE 24 PART A
// STATIC DATA REPORT — SHIP NAME
// ============================================================
void buildMsg24() {
  aisBitStream = "";
  //   0–  5  Message ID       (6b)
  //   6–  7  Repeat Indicator (2b)
  //   8– 37  MMSI             (30b)
  //  38– 39  Part Number      (2b) = 0 → Ship Name
  //  40–159  Ship Name        (120b = 20 × 6-bit)
  // 160–167  Spare            (8b)
  // Total: 168 bit
  addBits(24, 6);                         // Message ID
  addBits(0, 2);                          // Repeat Indicator
  addBits(AIS_MMSI, 30);                  // MMSI
  addBits(0, 2);                          // Part Number = 0 (name)
  addTextBits(SHIP_NAME, 20);             // Ship Name (20 char)
  addBits(0, 8);                          // Spare

  String payload = sixBitEncode(aisBitStream);

  strncpy(lastMsg24RawPayload, payload.c_str(), sizeof(lastMsg24RawPayload) - 1);
  lastMsg24RawPayload[sizeof(lastMsg24RawPayload) - 1] = '\0';

  char channel = (seqMsg24++ % 2 == 0) ? 'A' : 'B';
  snprintf(lastMsg24Payload, sizeof(lastMsg24Payload),
    "AIVDM,1,1,,%c,%s,0", channel, payload.c_str());

  char tmp[256];
  snprintf(tmp, sizeof(tmp), "!%s", lastMsg24Payload);
  uint8_t cs = nmeaChecksum(tmp);
  snprintf(lastMsg24FullNMEA, sizeof(lastMsg24FullNMEA),
    "!%s*%02X", lastMsg24Payload, cs);
...
void broadcastJSON() {
  ...
  if (newMsg24) {
    char ais24[280];
    snprintf(ais24, sizeof(ais24), "{\"ais_msg24\":\"%s\"}", lastMsg24FullNMEA);
    wsServer.broadcastTXT(ais24);
    newMsg24 = false;
  }
}
...
void setup() {
  ...
  DEBUG_SERIAL.println(" AIRCRAFT PROTOTIPE — AIS + WiFi + MSP    ");
  DEBUG_SERIAL.println("===========================================");
  ...
  DEBUG_SERIAL.printf(" Ship Name  : %s\n", SHIP_NAME);
  ...
  DEBUG_SERIAL.println(" MSG 24     : statis dgn nama \"" SHIP_NAME "\" tiap 6 menit");
  DEBUG_SERIAL.println("===========================================");
...
void loop() {
  ...
  // ---- AIS MSG24 (Static Data — Ship Name) ----
  if (now - tMsg24 >= INTERVAL_MSG24) {
    buildMsg24();
    AIT33_SERIAL.print(lastMsg24RawPayload);
    AIT33_SERIAL.print("\r\n");
    DEBUG_SERIAL.print("[MSG24→AIT33 payload] "); DEBUG_SERIAL.println(lastMsg24RawPayload);
    DEBUG_SERIAL.print("[MSG24→LOG   full   ] "); DEBUG_SERIAL.println(lastMsg24FullNMEA);
    strncpy(sdAis24Buf, lastMsg24FullNMEA, sizeof(sdAis24Buf) - 1);
    tMsg24 = now;
  }
  ...
  if (now - tSDLog >= INTERVAL_SD_LOG) {
    sdWriteLog(sdAis9Buf, sdAis8Buf, sdAis24Buf);
    sdAis9Buf[0] = '\0';
    sdAis8Buf[0] = '\0';
    sdAis24Buf[0] = '\0';
    tSDLog = now;
